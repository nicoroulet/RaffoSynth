...
asm=[map(int,x.strip().split()) for x in open("../data/asm_choto_256.out").readlines()]
...
plt.plot(run_cpp, label="run C", color='b', linewidth=1.5)
plt.plot(osc_cpp, ':', label="osciladores C", color='b', linewidth=2.5)
plt.plot(eq_cpp, '--', label="eq C", color='b', linewidth=1.5)
...
plt.savefig("../plots/tiempos_asm_vs_cpp_choto.png", bbox_inches='tight')
plt.show()